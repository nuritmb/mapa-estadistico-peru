...
def show_district_detail(row: pd.Series, level_key: str = "distrito",
    ...
    if level_key == "distrito":
        ...
        subtitle = f"Provincia · {row['DEPARTAMENTO'].title()}"
        icon = "🏙️"
    else:  # departamento
    ...
    col1.metric("Castillo (2ª vuelta)", f"{row['r2_pct_castillo']:.1f}%")
    col2.metric("Fujimori (2ª vuelta)", f"{row['r2_pct_fujimori']:.1f}%")
    margin = row["r2_margin"]
    winner = "Castillo" if margin > 0 else "Fujimori"
    col3.metric("Margen", f"{abs(margin):.1f} pp", delta=f"{winner} gana", delta_color="off")

    # ── 1st round bar ──
    st.markdown("**Primera vuelta — votos por candidato**")
    r1_data = []
    for abbr, (name, party) in CANDIDATES_R1.items():
        col = f"r1_pct_{abbr}"
        if col in row.index and not pd.isna(row[col]):
            r1_data.append({"Candidato": f"{name} ({abbr})", "Voto %": row[col], "abbr": abbr})
    r1_df = pd.DataFrame(r1_data).sort_values("Voto %", ascending=True)
    bar_colors = [PARTY_COLORS.get(r["abbr"], "#888") for _, r in r1_df.iterrows()]
    fig_bar = go.Figure(go.Bar(
        x=r1_df["Voto %"], y=r1_df["Candidato"],
        orientation="h",
        marker_color=bar_colors,
        text=r1_df["Voto %"].map(lambda v: f"{v:.1f}%"),
    ...
    fig_bar.update_layout(
        ...
        showlegend=False,
        plot_bgcolor="white",
        xaxis=dict(range=[0, r1_df["Voto %"].max() * 1.2]),
    )
    ...
    if not pd.isna(swing_val):
        ...
        st.markdown(
            f"**Swing Castillo (R1→R2):** {direction} `{swing_val:+.1f} pp`  "
            f"*(era {row['r1_pct_PL']:.1f}% en 1ª vuelta → {row['r2_pct_castillo']:.1f}% en 2ª)*"
        )

    # ── Census ──
    st.markdown("**Contexto socioeconómico (Censo 2017)**")
    census_items = [
        ("Pobreza total", "pct_pobreza_total", "%"),
        ("Pobreza extrema", "pct_pobreza_extrema", "%"),
        ("IDH 2019", "idh_2019", ""),
        ("Área rural", "pct_rural", "%"),
        ("Lengua indígena", "pct_indigenous_total", "%"),
        ("Altitud", "altitude", " m"),
    ]
    cols = st.columns(3)
    ...
    if not pd.isna(row.get("cvr_deaths", np.nan)):
        st.markdown("**Conflicto armado (CVR)**")
        c1, c2, c3 = st.columns(3)
        c1.metric("Muertes CVR (per cápita)", f"{row['cvr_deaths']:.4f}")
        c2.metric("Eventos violentos", f"{row['cvr_events']:.1f}")
        c3.metric("Zona de emergencia 1990", "Sí" if row.get("emergency_zone_1990", 0) == 1 else "No")
        if row.get("imputed", False):
            st.caption(
                f"⚠️ Datos de conflicto imputados espacialmente "
                f"(distrito creado después de 1975; {int(row.get('n_parents',1))} "
                f"distrito(s) padre)"
            )
...
def show_scatter(df, level_key: str = "distrito"):
    unit_plural = {"distrito": "distrito", "provincia": "provincia",
                   "departamento": "departamento"}[level_key]
    st.markdown(f"### Análisis de correlación por {unit_plural}")
    if level_key == "departamento":
        st.caption(
            "⚠️ Con n = 25 departamentos, las correlaciones son muy inestables. "
            "Preferí nivel distrito o provincia para análisis estadístico."
        )

    with st.expander("📚 ¿Qué significa esto? (guía rápida)", expanded=False):
        st.markdown(
            """
...
- La nube de puntos suele verse más "redonda" y la tendencia, más clara.
    ...
    x_key = c1.selectbox("Variable X (socioeconómica / conflicto / reforma agraria)",
                          options=list(all_x.keys()),
                          format_func=lambda k: all_x[k],
                          key="scatter_x")
    y_key = c2.selectbox("Variable Y (electoral)",
                          options=list(all_y.keys()),
                          format_func=lambda k: all_y[k],
                          key="scatter_y")
    color_by = c3.selectbox("Color por", ["Departamento", "Ganador R2", "Ninguno"],
                             key="scatter_color")

    # ── Robustness controls ───────────────────────────────────────────────────
    c4, c5 = st.columns([2, 2])
    trim_pct = c4.select_slider(
        "Recortar outliers (% en cada cola, ambos ejes)",
        options=[0.0, 0.5, 1.0, 2.5, 5.0, 10.0],
        value=0.0,
        help=(
            "Elimina los distritos que estén en los extremos X o Y. "
    ...
    show_outliers = c5.checkbox(
        "Mostrar puntos recortados como fantasmas", value=True, key="scatter_ghost"
    )

    # ── Log-scale toggles ─────────────────────────────────────────────────────
    c6, c7 = st.columns([2, 2])
    log_x = c6.checkbox(
        "📐 Escala log en X  —  log(X + 1)",
        value=False,
        key="scatter_log_x",
        help=(
            "Transforma X con el logaritmo natural ln(X + 1). "
    ...
    log_y = c7.checkbox(
        "📐 Escala log en Y  —  log(Y + 1)",
        value=False,
        key="scatter_log_y",
        help=(
            "Igual que X log, pero aplicado a la variable electoral del eje Y. "
    ...
    control_keys = st.multiselect(
        "🎛️ Controlar por (mantener constante)",
        options=list(control_options.keys()),
        format_func=lambda k: control_options[k],
        default=[],
        key="scatter_controls",
        help=(
            "Elige una o más variables a **mantener constantes** para aislar el "
            ...
            "(teorema de Frisch-Waugh-Lovell). Ejemplo: 'manteniendo pobreza fija, "
            "¿cuánto predicen las muertes CVR el voto por Castillo?'"
        ),
    )
    ...
    if log_x:
        ...
        if n_neg_x > 0:
            st.warning(
                f"⚠️ {n_neg_x} distrito(s) tienen valores negativos en X y no se pueden "
                f"transformar con log. Se excluyen del análisis."
            )
    ...
    if log_y:
        ...
        if n_neg_y > 0:
            st.warning(
                f"⚠️ {n_neg_y} distrito(s) tienen valores negativos en Y y no se pueden "
                f"transformar con log. Se excluyen del análisis."
            )
    ...
    color_col = None
    color_map = None
    if color_by == "Departamento":
        color_col = "DEPARTAMENTO"
    elif color_by == "Ganador R2":
        color_col = "r2_winner"
    ...
    if show_outliers and len(dropped) > 0:
        fig.add_trace(go.Scatter(
            ...
            marker=dict(color="#cccccc", size=6, opacity=0.5,
                        line=dict(width=0)),
            name=f"Recortados (n={len(dropped)})",
    ...
    if log_notes:
        st.info(
            "🔢 **Transformación aplicada:** " + " · ".join(log_notes)
            + ". Se usa logaritmo natural con un desplazamiento de +1 "
    ...
    if len(kept) > 2:
        ...
        def interpret_corr(v):
            a = abs(v)
            sign = "positiva" if v > 0 else "negativa"
            if a < 0.1:   strength = "nula"
            elif a < 0.3: strength = "débil"
            elif a < 0.5: strength = "moderada"
            elif a < 0.7: strength = "fuerte"
            else:         strength = "muy fuerte"
            if a < 0.1:
                return "sin relación"
            return f"{strength} y {sign}"

        # Label suffixes reflecting which transformations are active
        pearson_suffix_parts = []
        if log_x or log_y:
            pearson_suffix_parts.append("log")
        if trim_pct > 0:
            pearson_suffix_parts.append("recortado")
        pearson_suffix = f" — {', '.join(pearson_suffix_parts)}" if pearson_suffix_parts else ""
        spearman_suffix = " — recortado" if trim_pct > 0 else ""

        m1, m2, m3, m4 = st.columns(4)
        m1.metric(
            "Pearson r (lineal)" + pearson_suffix,
            f"{pearson_r:+.3f}",
            delta=f"{pearson_r - pearson_full:+.3f} vs. completo" if trim_pct > 0 else None,
            delta_color="off",
            help=(
                "Correlación de Pearson: mide la relación **lineal** entre los valores "
                "(crudos o transformados con log, según el toggle). Va de −1 a +1. "
                "Sensible a outliers y a la forma de la distribución."
            ),
        )
        m2.metric(
            "Spearman ρ (rangos)" + spearman_suffix,
            f"{spearman_rho:+.3f}",
            delta=f"{spearman_rho - spearman_full:+.3f} vs. completo" if trim_pct > 0 else None,
            delta_color="off",
            help=(
                "Correlación de Spearman: igual que Pearson, pero sobre los **rangos** "
                "(posiciones 1º, 2º, 3º…). Detecta relaciones monotónicas aunque no sean "
                "lineales y es robusta a outliers. **No cambia** al aplicar log — por eso "
                "sigue siendo la referencia más estable."
            ),
        )
        m3.metric(
            "n (distritos usados)", f"{len(kept):,}",
            help="Número de distritos que entran en el cálculo después del recorte."
        )
        m4.metric(
            "Recortados",
            f"{len(dropped):,}",
            ...
            delta_color="off",
            help=(
                "Distritos excluidos del cálculo por estar en los extremos (colas) "
        ...
        st.markdown(
            f"**Interpretación**: la relación entre *{all_x.get(x_key, x_key)}* "
            f"y *{all_y.get(y_key, y_key)}* es **{interpret_corr(spearman_rho)}** "
            f"(Spearman ρ = {spearman_rho:+.2f}, n = {len(kept):,})."
        )
        ...
        if gap > 0.1:
            st.warning(
                f"⚠️ **Pearson y Spearman difieren en {gap:.2f}.** "
            ...
            st.caption(
                "✓ Pearson y Spearman son similares → la relación es "
                "aproximadamente lineal y no está dominada por outliers."
            )

    # ── Partial / conditional regression (Frisch-Waugh-Lovell) ────────────────
    if control_keys and len(kept) > len(control_keys) + 2:
        st.divider()
        st.markdown("### 🎛️ Regresión parcial (manteniendo controles constantes)")

        try:
            import statsmodels.api as sm
        except ImportError:
            st.error(
                "La regresión parcial requiere `statsmodels`. "
                "Instálalo con `pip install statsmodels` y reinicia la app."
            )
            return
        ...
        av_fig.update_layout(
            ...
            xaxis=dict(title=f"Residuos de {x_label} | controles",
                       showgrid=True, gridcolor="#eee", zeroline=True, zerolinecolor="#bbb"),
            yaxis=dict(title=f"Residuos de {y_label} | controles",
                       showgrid=True, gridcolor="#eee", zeroline=True, zerolinecolor="#bbb"),
            legend=dict(yanchor="top", y=0.99, xanchor="right", x=0.99,
                        bgcolor="rgba(255,255,255,0.85)",
                        bordercolor="#ccc", borderwidth=1),
            title=dict(
                text=f"Gráfico de variable añadida — controlando por: {ctrl_labels}",
        ...
        p1.metric(
            "Correlación parcial (r)",
            f"{partial_r:+.3f}",
            delta=f"{partial_r - raw_r:+.3f} vs. bruta",
            delta_color="off",
            help=(
                "Pearson r calculado sobre los **residuos** de X e Y después de "
        ...
        p2.metric(
            ...
            f"{beta:+.3f}",
            help=(
                "Coeficiente de X en una regresión múltiple Y ~ X + controles. "
        ...
        p3.metric(
            ...
            f"{pval:.3g}",
            help=(
                "Probabilidad de observar |β| igual o mayor si el verdadero coeficiente "
        ...
        p4.metric(
            "n",
            f"{len(kept):,}",
            help=f"Observaciones con valores no-nulos en X, Y y los {len(control_keys)} controles.",
        )
        ...
        if abs(partial_r) < 0.05 and abs(raw_r) > 0.15:
            verdict = (
                "**La asociación bruta se desvanece al controlar por los "
                "controles.** Probablemente los controles explican la mayor parte "
                "de la relación aparente entre X e Y (variable de confusión / "
                "confounder)."
            )
        elif np.sign(partial_r) != np.sign(raw_r) and abs(partial_r) > 0.1:
            verdict = (
                "⚠️ **El signo de la relación se invierte al controlar.** Caso "
                "clásico de paradoja de Simpson: lo que parecía positivo (o "
                "negativo) sin controles cambia de dirección cuando los "
                "mantenemos constantes. Interpretar con mucho cuidado."
            )
        elif abs(partial_r) >= 0.9 * abs(raw_r):
            verdict = (
                "La asociación se mantiene prácticamente igual al controlar "
                "→ los controles no son confundidores de esta relación."
            )
        elif abs(partial_r) < abs(raw_r):
            verdict = (
                f"La asociación se **atenúa** ({raw_r:+.2f} → {partial_r:+.2f}) "
                "pero no desaparece: parte del efecto bruto sí se explica por "
                "los controles, y parte queda como efecto propio de X."
            )
        else:
            verdict = (
                f"La asociación se **refuerza** al controlar "
                f"({raw_r:+.2f} → {partial_r:+.2f}). Los controles estaban "
                "enmascarando una relación más clara (supresión)."
            )

        st.markdown(
            f"**Interpretación**: manteniendo constantes *{ctrl_labels}*, la "
            f"correlación parcial entre *{all_x.get(x_key, x_key)}* y "
            f"*{all_y.get(y_key, y_key)}* es **{interpret_corr(partial_r)}** "
            f"(r = {partial_r:+.2f}, β = {beta:+.3f}, p = {pval:.3g}).  \n"
            + verdict
        )

        with st.expander("ℹ️ ¿Qué es una regresión parcial? (detalle técnico)"):
            st.markdown(
                """
...
**Cuidados**:
- Esto **no** es causalidad. Si hay un confundidor que no has incluido, la
  estimación sigue sesgada.
- Añadir demasiados controles correlacionados con X causa **colinealidad**:
  los errores estándar crecen y todo pierde significancia.
- Si controlas por un **mediador** (algo que está en el camino causal X→Z→Y),
  "bloqueas" el efecto real. Pensá bien la dirección causal antes de controlar.
                """
            )
...
def show_national_totals(df):
    ...
    cols = st.columns(6)
    cols[0].metric("Castillo (nacional)", f"{cast_pct:.2f}%")
    cols[1].metric("Fujimori (nacional)", f"{fuji_pct:.2f}%")
    cols[2].metric("Margen", f"{cast_pct - fuji_pct:.2f} pp")
    cols[3].metric("Distritos Castillo", f"{cast_distr:,}")
    cols[4].metric("Distritos Fujimori", f"{fuji_distr:,}")
    cols[5].metric("Votos válidos (2ª v.)", f"{total_valid:,.0f}")
